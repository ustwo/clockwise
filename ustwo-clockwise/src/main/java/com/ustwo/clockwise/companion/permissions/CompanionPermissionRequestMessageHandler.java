    ...
    public void onMessageReceived(String path, DataMap map, WearableAPIHelper apiHelper) {
        if(path.equals(Constants.DATA_PATH_COMPANION_PERMISSION_REQUEST)) {
            ...
            if(request != null) {
                ...
                // send back empty data map if we receive request with bad data
                apiHelper.putMessage(Constants.DATA_PATH_COMPANION_PERMISSION_RESPONSE, new DataMap().toByteArray(), null);
            }
        }
    }
